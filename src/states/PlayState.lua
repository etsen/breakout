...
function PlayState:enter(params)
   ...
   self.paddlePoints = params.paddlePoints
    ...
    self.powerup = { [1] = Powerup(-5, -5, 4)}
       --tracks how many times the ball has had a collision. 100 - ball_hits_block gives the number

       ball_hits_block =  math.floor(self.health/3 * HIT_MAX) 
end
...
function PlayState:update(dt)
   ...
   for k, ball in pairs(self.ball) do
    if ball:collides(self.paddle) then
        ...
        elseif ball.x > self.paddle.x + (self.paddle.width / 2) 
        and self.paddle.dx > 0 then
    ...
    -- detect if the powerup has been picked                    -- Powerup 4 is ball multiplier
     for k, powerup in pairs(self.powerup) do                         -- Powerup 10 is key
        if powerup:collide(self.paddle) then             -- reset powerup and remove it from the table
    ...
    for k, brick in pairs(self.bricks) do
        ...
        if brick.inPlay and ball:collides(brick) then
         ...
         -- powerups spawning by ball_hits_block (ball hit brick)
            ball_hits_block = ball_hits_block - 1
            if math.random(ball_hits_block) > 30 then                      -- this increases the probability of spawning after every hit
                table.insert(self.powerup, Powerup(brick.x, brick.y, 4)) --multiball
                ball_hits_block =   math.floor(self.health/3 * HIT_MAX) -- reset ball_hits_block
            elseif lockedBrick == true and math.random(math.floor(ball_hits_block/2)) == math.floor(ball_hits_block/2)  then
                table.insert(self.powerup, Powerup(brick.x, brick.y, 10)) --keys
                ball_hits_block =   math.floor(self.health/3 * HIT_MAX) 
            end
...
function PlayState:render()
    ...
    --love.graphics.print(tostring(ball_hits_block),5,20)
